...
**source.md内に明示的な指示がある場合はそれに従ってください。** 例:
- `---` によるスライド区切り指定 → そのまま尊重
- レイアウト指定（「2カラムで」「カードで」等） → 指定通りのレイアウトを使用
- 特定のスライドクラス指示（「引用スライドにする」等） → 指定通り適用
...
- **タイトル/サブタイトル**: コンテンツの見出しや主題から抽出（source.mdに明記されていればそれを使用）
- **スライド分割**: source.mdに `---` があればそれに従う。なければ意味的なまとまりで分割
- **レイアウト選択**: source.mdに指示があればそれに従う。なければ内容に最適なパターンを自動適用
